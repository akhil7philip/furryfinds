...
const TEMPLATE_REPO = 'https://github.com/akhil7philip/furryfinds.git'
...
async function main() {
  ...
  /* 1. inputs */
  const brand = (await ask('Brand name (e.g. Furry Finds): ')) || 'My Site'
  const domain = (await ask('Domain (e.g. furryfinds.club): ')) || 'example.com'
  const niche = (await ask('Niche (e.g. Pet Accessories): ')) || 'General'
  ...
  replaceInDir(tmpDir, [
    ['Furry Finds', brand],
    ['furryfinds.club', domain],
    ['https://furryfinds.club', siteUrl],
    ['Pet Accessories', niche],
    ['Pet Accessories', niche],
    ['pet accessories', niche.toLowerCase()],
